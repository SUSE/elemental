...
`
	...
	It("Successfully applies configurations to output directory", func() {
		...
		Expect(r.ResolvedManifest).To(Equal(activeReleaseManifest))
		Expect(len(r.CustomPartitions)).To(Equal(1))
		Expect(r.CustomPartitions[0].Label).To(Equal(PrepareLabel))
		Expect(r.CustomPartitions[0].MountPoint).To(Equal(PrepareMnt))
		Expect(r.CustomPartitions[0].Size).To(Equal(PrepareSize))
		...
		Expect(err).ToNot(HaveOccurred())
		_, err = fs.Stat(filepath.Join(outputDir.OverlaysDir(), PrepareMnt, "network", "nmstate1.yaml"))
		Expect(err).ToNot(HaveOccurred())
	...
	It("Fails to configure systemd extensions", func() {
		...
		Expect(r).To(BeNil())
		Expect(err).To(HaveOccurred())
		Expect(err).To(MatchError("configuring system extensions: filtering enabled systemd extensions: extension(s) not found: [{missing}]"))
	})
})
