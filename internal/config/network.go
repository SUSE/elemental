...
func (m *Manager) configureNetworkOnPartition(conf *image.Configuration, outputDir OutputDir, p *deployment.Partition) error {
	if !needsNetworkSetup(conf) {
		m.system.Logger().Info("Network configuration not provided, skipping.")
		return nil
	}

	netDir := filepath.Join(outputDir.OverlaysDir(), p.MountPoint, "network")
