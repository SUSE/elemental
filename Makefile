...
PLATFORM?=linux/$(ARCH)
IMG?=$(BUILD_DIR)/elemental-$(FLAVOR)-$(ARCH)
...
.PHONY: toolkit-container
toolkit-container:
	$(DOCKER) build --platform $(PLATFORM) --target runner --tag $(TOOLKIT_REPO):$(VERSION) .

.PHONY: build-disk
build-disk: $(BUILD_DIR) toolkit-container
	...
	$(DOCKER) run --rm \
		...
		--debug install --os-image $(OS_REPO):$(OS_VERSION) --target $${TARGET} --config /build/config.sh
	BUILD_ERR=$$?
