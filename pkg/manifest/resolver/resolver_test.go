...
func validateResolvedManifest(rm *resolver.ResolvedManifest, coreOnly bool) {
	...
	Expect(rm.CorePlatform.Components.Helm.Charts[0].Values).To(Equal(map[string]any{"image": map[string]any{"tag": "latest"}}))
	Expect(len(rm.CorePlatform.Components.Helm.Charts[0].DependsOn)).To(Equal(1))
	Expect(rm.CorePlatform.Components.Helm.Charts[0].DependsOn[0].Name).To(Equal("baz"))
	Expect(rm.CorePlatform.Components.Helm.Charts[0].DependsOn[0].Type).To(BeEquivalentTo("helm"))
