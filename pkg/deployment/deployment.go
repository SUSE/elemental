...
type PartFunction int

const (
	EFI PartFunction = iota + 1
	System
	Recovery
	Data
)
...
func ParseFunction(function string) (PartFunction, error) {
	switch function {
	case "efi":
		return EFI, nil
	case "system":
	...
	case "data":
		return Data, nil
	default:
		return PartFunction(0), fmt.Errorf("unknown partition function: %s", function)
	}
}

func (p PartFunction) String() string {
	switch p {
	case EFI:
		return "efi"
	case System:
...
func (p PartFunction) MarshalJSON() ([]byte, error) {
	return json.Marshal(p.String())
}

func (p *PartFunction) UnmarshalJSON(data []byte) (err error) {
	var function string
	if err = json.Unmarshal(data, &function); err != nil {
		return err
	}
...
type Partition struct {
	...
	Function    PartFunction `json:"function"`
...
func checkRecoveryPart(s *sys.System, d *Deployment) error {
	...
	for _, disk := range d.Disks {
		for _, part := range disk.Partitions {
			if part.Function == Recovery && !found {
				found = true
			...
			} else if part.Function == Recovery {
