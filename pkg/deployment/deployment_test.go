...
var _ = Describe("Deployment", Label("deployment"), func() {
	...
	It("fails if multiple recovery partitions are set", func() {
		d := deployment.DefaultDeployment()
		d.Disks[0].Partitions = append(d.Disks[0].Partitions, []*deployment.Partition{
			{Function: deployment.Recovery}, {Function: deployment.Recovery},
		}...)
		err = d.Sanitize(s)
		Expect(err).To(HaveOccurred())
	...
	It("fails if no system partition is defined", func() {
		d := &deployment.Deployment{
			Disks: []*deployment.Disk{
				{Partitions: []*deployment.Partition{{Function: deployment.Data}}},
			},
		}
		err = d.Sanitize(s)
		Expect(err).To(HaveOccurred())
	...
	It("fails if no efi partition is defined", func() {
		d := &deployment.Deployment{
			Disks: []*deployment.Disk{
				{Partitions: []*deployment.Partition{{Function: deployment.System}}},
			},
		}
		err = d.Sanitize(s)
		Expect(err).To(HaveOccurred())
		...
		d := &deployment.Deployment{
			Disks: []*deployment.Disk{
				{Partitions: []*deployment.Partition{
					{Function: deployment.System},
					{Function: deployment.EFI, RWVolumes: []deployment.RWVolume{{Path: "/some/path"}}},
					{Function: deployment.Data, Size: deployment.AllAvailableSize},
				}},
			},
		}
		Expect(d.Sanitize(s)).To(Succeed())
